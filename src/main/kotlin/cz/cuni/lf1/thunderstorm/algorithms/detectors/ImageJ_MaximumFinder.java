...
public class ImageJ_MaximumFinder implements ExtendedPlugInFilter, DialogListener {
    //filter params
     ...
     */
    public long[] getSortedMaxPoints(ImageProcessor ip, ByteProcessor typeP, boolean excludeEdgesNow,
     ...
     * @param outputType
     */
    public void analyzeAndMarkMaxima(ImageProcessor ip, ByteProcessor typeP, long[] maxPoints, boolean excludeEdgesNow,
     ...
     */
    public ByteProcessor make8bit(ImageProcessor ip, ByteProcessor typeP, boolean isEDM, float globalMin, float globalMax, double threshold) {
        byte[] types = (byte[])typeP.getPixels();
        double minValue;
        if (isEDM) {
            threshold = 0.5;
     ...
     */
    public void cleanupMaxima(ByteProcessor outIp, ByteProcessor typeP, long[] maxPoints) {
        byte[] pixels = (byte[])outIp.getPixels();
        byte[] types = (byte[])typeP.getPixels();
        int nMax = maxPoints.length;
     ...
     */
    public void cleanupExtraLines(ImageProcessor ip) {
     ...
     */
    public void watershedPostProcess(ImageProcessor ip) {
     ...
     */
    public boolean watershedSegment(ByteProcessor ip) {
